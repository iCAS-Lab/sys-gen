define_design_lib WORK -path "./work/spike_speed"

set TopDesign "spiking_systolic_array_32x32"

set_svf ./net/syn/spike_speed/${TopDesign}.svf
...
report_clock -attributes -skew  > ./reports/spike_speed/report_clock.txt
report_hierarchy                > ./reports/spike_speed/report_hier.txt
report_compile_options          > ./reports/spike_speed/report_option.txt
report_resources -hierarchy     > ./reports/spike_speed/report_resource.txt
report_port -verbose            > ./reports/spike_speed/report_port.txt
all_registers -level_sensitive  > ./reports/spike_speed/report_latches.txt
report_timing -loops            > ./reports/spike_speed/report_loops.txt
report_power                    > ./reports/spike_speed/report_power.txt

report_timing > reports/spike_speed/report_timing.txt

# maximum transition violation
report_constraint -all_violators -max_transition -nosplit -significant_digits 4 \
                                > ./reports/spike_speed/report_maxttransitions.txt

# maximum fanout violation
report_constraint -all_violators -max_fanout -nosplit -significant_digits 4 \
                                > ./reports/spike_speed/report_maxfanout.txt

# setup violation report
report_constraint -all_violators -max_delay -nosplit -significant_digits 4 \
                                > ./reports/spike_speed/report_vio_max_simple.txt

report_constraint -all_violators -max_delay -nosplit -significant_digits 4 -verbose \
                                > ./reports/spike_speed/report_vio_max_verbose.txt


report_area     > ./reports/spike_speed/report_area.txt
...
write_file -format ddc     -output ./net/syn/spike_speed/$TopDesign.ddc  -hierarchy

write_file -format verilog -output ./net/syn/spike_speed/$TopDesign.syn.v -hierarchy

write_sdc net/syn/spike_speed/$TopDesign.sdc

write_sdf net/syn/spike_speed/$TopDesign.sdf
